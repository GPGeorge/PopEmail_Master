...
namespace OpenPopNoUI.TestApplication
{
    /// <summary>
    ...
    //public class TestForm : Form
    {
        ...
        private TestForm()
        {
            ...
            if (File.Exists(file))
            {
                using (StreamReader reader = new StreamReader(File.OpenRead(file)))
                {
                    ...
                    string strServer = reader.ReadLine(); // Hostname
                    string strport = reader.ReadLine(); // Port
                    bool booSsl = bool.Parse(reader.ReadLine() ?? "true"); // Whether to use SSL or not
                    string strlogin = reader.ReadLine(); // Username
                    string strpassword = reader.ReadLine(); // Password
        ...
        //GPC Remove the form elements 2016_12_07
        ...
        {
            //GPC 2016_12_07 disable form related elements
            ...
            try
            {
                ...
                if (File.Exists(file))
                {
                    using (StreamReader reader = new StreamReader(File.OpenRead(file)))
                    {
                        // This describes how the OpenPOPLogin.txt file should look like
                        string strServer = reader.ReadLine(); // Hostname
                        int intport = int.Parse(reader.ReadLine()); // Port
                        bool booSsl = bool.Parse(reader.ReadLine() ?? "true"); // Whether to use SSL or not
                        string strlogin = reader.ReadLine(); // Username
                        string strpassword = reader.ReadLine();


                        if (pop3Client.Connected)
                            pop3Client.Disconnect();
                        pop3Client.Connect(strServer, intport, booSsl);
                        pop3Client.Authenticate(strlogin, strpassword);
